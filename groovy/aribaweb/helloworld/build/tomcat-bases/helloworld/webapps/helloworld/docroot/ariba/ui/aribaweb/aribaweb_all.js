...
var Util={bind:function(f,oCtx){return $.proxy(f,oCtx)
...
},itemOrArrAdd:function(orig,obj){return !orig?obj:($.isArray(orig)?orig.push(obj):[orig].push(obj))
},arrayIndexOf:function(a,o){$.inArray(a,o)
},arrayAddIfNotExists:function(a,o){if($.inArray(a,o)<0){a.push(o)
...
},indexOf:function(a,o){$.inArray(a,o)
...
ariba.Dom=function(){var H=ariba.Util;
var N="MSIE 5.0";
var D="Netscape";
var K=1;
var J=new Array();
var B=navigator.appVersion;
var Q=(window.attachEvent&&!window.opera)?true:false;
var O=B.indexOf("MSIE 9")!=-1;
var P=B.indexOf("MSIE 8")!=-1;
var R=B.indexOf("MSIE 7")!=-1;
var S=B.indexOf("MSIE 6")!=-1;
var I=P||O;
var M=R||I;
var C=S||M;
var F="below";
...
var L="above";
var G=[F,E,U,T,L];
var A={IsIE:Q,IsIE9:O,IsIE8:P,IsIE7:M,IsIE6Only:S,IsIE6:C,IsIEonMac:(Q&&(navigator.platform!="Win32")&&(navigator.platform!="Win64"))?true:false,IsNS6:(!document.all&&document.getElementById)?true:false,IsMoz:(!document.all&&document.getElementById)?true:false,isChrome:navigator.appVersion.indexOf("Chrome")!=-1,isSafari:navigator.appVersion.indexOf("Safari")!=-1,isIPad:navigator.platform.indexOf("iPad")!=-1,AWEmptyDocScriptlet:S?"javascript:false":"javascript:void(0);",ApplicationType:"",AWOpenWindowErrorMsg:null,getElementById:function(X){var V=null;
try{if(X&&X!=null&&X!=""){V=document.getElementById?document.getElementById(X):document.all[X]
}}catch(W){}return V
},getElementsBy:function(a,W,c,Z){var V=[],b=a.getElementsByTagName(W),Y=0,X=b.length;
for(;
Y<X;
++Y){if(c(b[Y])){if(Z){return b[Y]
}else{V.push(b[Y])
}}}return V
},getElementBy:function(W,V,X){return A.getElementsBy(W,V,X,true)||null
},getElementsByClassName:function(a,W,X){var V=[],b;
if(a.getElementsByClassName){b=a.getElementsByClassName(W);
if(X){for(var Z=0,Y=b.length;
Z<Y;
++Z){if(X===b[Z].tagName){V.push(b[Z])
}}return V
}else{return b
}}else{return A.getElementsBy(a,X||"*",function(c){return A.hasClass(c,W)
})
}},getDocumentElementById:function(V,W){if(!V){return null
}return V.getElementById?V.getElementById(W):V.all[W]
},findParent:function(V,Y,X){var W=X?V:V.parentNode;
while(W!=null&&W.nodeName!=Y){W=W.parentNode
}return W
},findChild:function(V,a,Y){var X=null;
if(!V){return V
}else{if(Y&&(V.nodeName==a)){return V
}else{if(V.childNodes){var Z=V.childNodes;
for(var W=0;
W<Z.length&&X==null;
W++){X=this.findChild(Z[W],a,true)
}}}}return X
},findParentUsingPredicate:function(V,Y,X){var W=X?V:V.parentNode;
while(W!=null&&!Y(W)){W=W.parentNode
}return W
},findChildUsingPredicate:function(V,a,Y){var X=null;
if(!V){return V
}else{if(Y&&a(V)){return V
}else{if(V.childNodes){var Z=V.childNodes;
for(var W=0;
W<Z.length&&X==null;
W++){X=this.findChildUsingPredicate(Z[W],a,true)
}}}}return X
},find:function(W,V){return this.findChildrenUsingPredicate(W,function(X){return X.className&&X.className.indexOf(V)>=0
})
},findChildrenUsingPredicate:function(W,b,Z){var V=[];
if(!W){return W
}else{if(Z&&b(W)){return[W]
}else{if(W.childNodes){var a=W.childNodes;
for(var X=0;
X<a.length;
X++){var Y=this.findChildrenUsingPredicate(a[X],b,true);
V=V.concat(Y)
}}}}return V
},elementInDom:function(V){var W=this.documentElement();
while(V&&V!=W){V=V.parentNode
}return V==W
},appendToBody:function(V){if(document&&document.body){document.body.appendChild(V)
}else{alert("unable to find document / document.body")
}},_appendChildren:function(X,V){for(var W=0;
W<V.length;
W++){X.appendChild(V[W])
}},getInnerText:function(V){var W;
if(V){if(V.innerText){W=V.innerText
}else{if(V.textContent){W=V.textContent
}else{W=V.innerHTML.replace(/<[^>]+>/g,"");
W=W.replace(/&amp;/g,"&");
W=W.replace(/&lt;/g,"<");
W=W.replace(/&gt;/g,">")
}}}return W
},setInnerText:function(W,V){if(W){if(W.innerText||W.innerText==""){W.innerText=V
}else{W.innerHTML=V
}}},copyInnerText:function(W,V){if(W&&V){this.setInnerText(V,this.getInnerText(W))
}},isNetscape:function(){return(navigator.appName.indexOf(D)!=-1)
},openWindow:function(Y,V,X){var W=null;
try{W=J[V];
if(W==null||W.closed){if(X!=null){W=window.open(Y,V,X)
}else{W=window.open(Y,V)
}J[V]=W;
if(navigator.appVersion.indexOf(N)!=-1||navigator.appName.indexOf(D)!=-1){W.focus()
}else{if(this.IsIE6){function b(){W.focus()
}setTimeout(b)
}}}else{if(navigator.appVersion.indexOf(N)!=-1||navigator.appName.indexOf(D)!=-1){W.focus();
W.location.href=Y
}else{W.close();
if(X!=null){W=window.open(Y,V,X)
}else{W=window.open(Y,V)
}}}}catch(Z){function a(){alert(A.AWOpenWindowErrorMsg)
}setTimeout(a)
}return W
},formForName:function(V){return document.forms[V]
},addFormField:function(W,X,V){return this.addFormFieldWithId(W,X,X,V)
},addFormFieldWithId:function(Y,V,Z,W){if(document.getElementById){var X=Y[V];
if(X==null){X=document.createElement("input");
X.type="hidden";
X.id=V;
X.name=Z
}if(X.parentNode!=Y){Y.appendChild(X)
}X.value=W;
return X
}},removeFormField:function(V,X){var W=V[X];
if(W&&W.parentNode){W.parentNode.removeChild(W);
if(ariba.Dom.IsMoz){delete V[X]
}else{V[X]=null
}}},lookupFormId:function(W){var X=this.findParentUsingPredicate(W,function(Z){return Z.tagName=="FORM"||A.hasClass(Z,"formProxy")
});
var Y=null;
if(X){var V=X.getAttribute("_fn");
if(V!=null){X=this.formForName(V)
}Y=X.id
}return Y
},limitTextLength:function(a,X){if(X<1){return 
}var W=a.value;
if(this.IsMoz){W=W.replace(/([^\r])\n/g,"$1\r\n");
W=W.replace(/^\n/g,"\r\n")
}var b=W.length;
if(b>X){a.value=W.substring(0,X)
}else{var V=this.getElementById(a.id+"MLI");
if(!V){return 
}var Y=V.clientWidth;
V.style.width="";
V.innerHTML=X-b;
var Z=V.clientWidth;
V.style.width=Math.max(Y,Z)+"px"
}},addClass:function(W,V){if(W.nodeType==K){if(W.className!=""&&V&&V.charAt(V.length-1)!=" "){V=V+" "
}if(W.className&&W.className.indexOf(V)==-1){W.className=V+W.className
}else{if(!W.className){W.className=V
}}}},removeClass:function(Z,Y){var V=Z.className;
if(!Y||!V){return 
}var W=V.indexOf(Y);
if(W!=-1){var X=W+Y.length;
if(X<V.length&&V.charAt(X)==" "){X++
}Z.className=V.substring(0,W)+V.substring(X,V.length)
}},hasClass:function(W,V){return W.className&&(W.className.match(new RegExp("(^|\\s)"+V+"(\\s|$)"))!=null)
},positionDialogBox:function(Y){var V=this.positioningParent(Y.parentNode);
var X=(V==this.documentElement())?this.documentClientHeight():V.clientHeight;
var W=(V==this.documentElement())?this.documentClientWidth():V.clientWidth;
Y.style.left=W/2-Y.offsetWidth/2+this.getScrollLeft(V)+"px";
Y.style.top=X/2-Y.offsetHeight/2+this.getScrollTop(V)+"px";
if(Y.onresize){Y.onresize.call(Y)
...
}},setOpacity:function(W,V){var X=W.style;
if(Q){X.filter="alpha(opacity="+V+")"
}else{X.opacity=(parseInt(V)/100)
}},absoluteTop:function(X){var W=X.offsetTop;
var V=X.parentNode;
var Y=X.offsetParent;
while(V!=null&&Y!=null&&V!=this.getPageScrollElement()){if(V==Y){W+=V.offsetTop;
Y=V.offsetParent
}W-=V.scrollTop;
V=V.parentNode
}return W
},clientHeight:function(W,X){var V=W.clientHeight;
if(!X){X=W
}V+=X.scrollTop;
return V
},visibleInScrollArea:function(c){var Z=this.documentElement();
var b=this.getPageScrollElement();
var X=b.scrollTop;
var V=this.clientHeight(Z,b);
var Y=this.absoluteTop(c);
var d=Y+c.clientHeight;
var W=(Y>X)&&(V>Y);
var a=(d>X)&&(V>d);
return W||a
},isElementInViewport:function(W){var V=this._viewportState(W);
return V!=L&&V!=F
},setViewportState:function(W){var V=this._viewportState(W);
this.setState(W,V,G)
},_viewportState:function(X){var Y=X.getBoundingClientRect();
var V=this.getWindowSize();
var a=Y.top;
var W=Y.bottom;
var Z=V[1];
if(a>Z){return F
}else{if(a<=Z&&W>Z){return E
}else{if(a<0&&W<=Z){return T
}else{if(W<0){return L
}}}}return U
},clientWidth:function(V,W){var X=V.clientWidth;
if(!W){W=V
}X+=W.scrollLeft;
return X
},correctForRightEdge:function(V,Y){var X=this.clientWidth(document.documentElement,this.getPageScrollElement());
var W=X-Y.offsetWidth;
if(V>W){V=W
}return V
},correctForBottomEdge:function(X,Y){var V=this.clientHeight(document.documentElement,this.getPageScrollElement());
var W=V-Y.offsetHeight;
if(X>W){X=W
}return X
},repositionDivToWindow:function(Z){if(Z.className.indexOf("noReposition")==-1){var Y=this.absoluteTop(Z);
var W=this.absoluteLeft(Z);
var X=this.correctForBottomEdge(Y,Z);
var V=this.correctForRightEdge(W,Z);
this.setAbsolutePosition(Z,V,X)
}},absoluteLeft:function(X){var W=X.offsetLeft;
var V=X.parentNode;
var Y=X.offsetParent;
while(V!=null&&Y!=null&&V!=this.getPageScrollElement()){if(V==Y){W+=V.offsetLeft;
Y=V.offsetParent
}W-=V.scrollLeft;
V=V.parentNode
}return W
},minInsetWidth:function(Y,X){var W=this.absoluteLeft(Y);
var V=W+Y.offsetWidth;
var a=this.absoluteLeft(X);
var Z=a+X.offsetWidth;
return Math.min((a-W),(V-Z))
},cssToJSName:function(a){if(a.indexOf("-")<0){return a
}var Y=a.split("-");
var V=Y[0];
for(var W=1,X=Y.length;
W<X;
W++){var Z=Y[W];
V+=Z.charAt(0).toUpperCase()+Z.substring(1)
}return V
},effectiveStyle:function(W,Z){if(!W.style){return 
}var V=this.cssToJSName(Z);
var Y=W.style[V];
if(!Y){if(document.defaultView&&document.defaultView.getComputedStyle){var X=document.defaultView.getComputedStyle(W,null);
Y=X?X.getPropertyValue(Z):null
}else{if(W.currentStyle){Y=W.currentStyle[V]
}}}return Y=="auto"?null:Y
},offsetParent:function(V){if(V.offsetParent){return V.offsetParent
}if(V==document.body){return V
}while((V=V.parentNode)&&V!=document.body){if(this.effectiveStyle(V,"position")!="static"){return V
}}return document.body
},positioningParent:function(V){while(V&&(V=V.parentNode)&&V!=document.documentElement){var W=this.effectiveStyle(V,"position");
if(W=="absolute"||W=="relative"||W=="fixed"){break
}}return V||document.documentElement
},contentParent:function(V){var W=this.positioningParent(V);
return W==document.documentElement?document.body:W
},containerParent:function(W){var V=this.findParentUsingPredicate(W,function(X){return A.hasClass(X,"panelContainer")
});
return V!=null?V:document.body
},setAbsolutePosition:function(V,Y,X){var W=this.offsetParent(V);
if(W){Y-=this.absoluteLeft(W);
X-=this.absoluteTop(W);
if(W!=this.getPageScrollElement()){Y+=W.scrollLeft;
X+=W.scrollTop
}}V.style.left=Y+"px";
V.style.top=X+"px"
},relativeOffset:function(W){var Y,X=0;
var V=false;
do{Y+=W.offsetTop||0;
X+=W.offsetLeft||0;
W=W.offsetParent;
if(W){var Z=this.effectiveStyle(W,"position");
V=(Z=="absolute"||Z=="relative")
}}while(W&&!V);
return[X,Y]
},isVisible:function(V){var W=this.documentElement();
while(V!=W){if(V.style.display=="none"){return false
}V=V.parentNode
}return true
},containerOffsetSize:function(Y){var X;
var W=Y.offsetWidth,V=Y.offsetHeight;
if(!W){X=Y.style.display;
Y.style.display="BLOCK";
W=Y.offsetWidth;
V=Y.offsetHeight;
Y.style.display=X
}return[W,V]
},getWindowSize:function(){var W=0,V=0,X=0;
...
}}}return new Array(W,V,X)
},isWindowNarrow:function(){var V=this.getWindowSize();
return(V[0]<300)
},fadeInElement:function(V){this.fadeElement(V,"hidden","visible")
},fadeOutElement:function(V){this.fadeElement(V,"visible","hidden")
},fadeElement:function(W,V,Y){try{if(W.filters){if(!W.filters.blendTrans){W.style.filter="blendTrans(duration=.2)"
}W.style.visibility=V;
if(W.filters.blendTrans){if(W.filters.blendTrans.status!=2){W.filters.blendTrans.apply();
W.style.visibility=Y;
W.filters.blendTrans.play()
}}}}catch(X){W.style.visibility=Y
}},setBodyClass:function(V){if(this.IsIE){V+=" IsIE";
V+=this.IsIE7?" IsIE7":" IsIE6"
}else{if(this.isSafari){V+=" IsSaf";
if(this.isIPad){V+=" IsIPad"
}if(this.isChrome){V+=" IsChr"
}}else{V+=" IsMoz"
}}this.addClass(document.body,V)
},findRow:function(Y,V){var Z;
var X=Y.childNodes;
for(var W=0;
W<X.length;
W++){if(X[W].id==V){Z=X[W];
...
},findRowIndex:function(W,Z){var V=-1;
var Y=Z.rows;
for(var X=0;
X<Y.length;
X++){if(Y[X].id==W){V=X;
...
},findElement:function(V,X){if(!V){return null
}for(var W=0;
W<V.length;
W++){if(V[W].id==X){return V[W]
}}return null
},getHashLocation:function(){var V=window.location.hash;
if(!V||V==""||V=="#"){return null
}else{if(V.charAt(0)=="#"){return V.substring(1)
}else{return V
}}},boolAttr:function(Y,V,X){var W=Y.getAttribute(V);
return(W)?(W=="true"):X
},relocateDiv:function(Z,a){if(Z.getAttribute("_reloc")=="1"){var X=Z.id;
this.removeRelocatedCopy(X);
var V=this.findParentUsingPredicate(Z,function(c){return c.className&&c.className.indexOf("relocdest")!=-1
},true);
if(V){var W=document.createElement("div");
W.id=X+"_MovedCopy";
if(a){var Y=X+"_OrigLocation";
var b=document.createElement("div");
var Y=X+"_OrigLocation";
b.id=Y;
Z.parentNode.replaceChild(b,Z)
}else{Z.parentNode.removeChild(Z)
}V.appendChild(W);
W.appendChild(Z);
Z.setAttribute("_reloc",0)
}}},revertRelocatedCopy:function(Y){var V=this.getElementById(Y+"_MovedCopy");
if(V){var X=this.getElementById(Y+"_OrigLocation");
if(X){V.parentNode.removeChild(V);
var W=V.firstChild;
W.setAttribute("_reloc","1");
X.parentNode.replaceChild(W,X)
}}},removeRelocatedCopy:function(X){var W=X+"_MovedCopy";
var V=this.getElementById(W);
if(V){V.parentNode.removeChild(V)
}},elementValue:function(V){var W=this.getElementById(V);
return W?W.value:null
},elementIntValue:function(V){var W=this.getElementById(V);
return W?parseInt(W.value):0
},setElementValue:function(V,W){if(V){var X=this.getElementById(V);
if(X){X.value=W
}}},elmBottom:function(W){var V=W;
while(V=V.nextSibling){if(V.offsetTop){return V.offsetTop
}}return W.offsetTop+W.offsetHeight
},getRowCells:function(V){var W=V.children;
if(W==null){W=V.cells
}return W
},overlay:function(V){},unoverlay:function(V){},getActiveElementId:function(){var V=document.activeElement;
if(V){return V.id
}return null
},getScrollTop:function(V){return V.scrollTop
},getScrollLeft:function(V){return V.scrollLeft
},getPageScrollElement:function(){return document.documentElement
},checkWindowScrollbar:function(X,V){if(X){var W=document.documentElement;
V=(W.scrollHeight>W.clientHeight)
}if(V&&document.documentElement.style.overflowY!="scroll"){document.documentElement.style.overflowY="scroll";
return true
}if(!V&&document.documentElement.style.overflowY!="hidden"){document.documentElement.style.paddingRight="0px";
document.documentElement.style.overflowY="hidden";
return true
}return false
},setState:function(f,a,Y){if(!f){return 
}var g=f.className;
if(!g){return 
}if(!Y){Y=[a]
}var c,b,X,h,W,e,V;
...
b++){X=Y[b];
if(X==a){V=true
}e=this._isStateClass(W,X);
if(e){break
}}if(!e){Z.push(W);
if(V){h=W+"-"+a;
Z.push(h)
}}}f.className=Z.join(" ")
},unsetState:function(b,W){if(!b){return 
}var c=b.className;
if(!c){return 
}var Z,Y,d,V;
var X=[];
...
Z<a.length;
Z++){V=a[Z];
if(!this._isStateClass(V,W)){X.push(V)
}}b.className=X.join(" ")
},_isStateClass:function(W,V){return W.indexOf("-"+V)>=0
},createElement:function(W){var V=document.createElement("div");
V.innerHTML=W;
return V.firstChild
},EOF:0};
if(A.isSafari){H.extend(A,function(){return{getScrollTop:function(V){var W=V.scrollTop;
if(V=document.documentElement){W=document.body.scrollTop
}return W
},getScrollLeft:function(V){var W=V.scrollLeft;
if(V=document.documentElement){W=document.body.scrollLeft
}return W
},getPageScrollElement:function(){return document.body
},EOF:0}
}())
}if(A.IsIE){H.extend(A,function(){var V;
var W=function(){ariba.Dom.overlay(this)
};
return{setPageScroll:function(Y,X){this.setPageScrollLeft(Y);
this.setPageScrollTop(X)
},setPageScrollTop:function(X){this.documentElement().scrollTop=X
},getPageScrollTop:function(){return this.documentElement().scrollTop
},setPageScrollLeft:function(X){this.documentElement().scrollLeft=X
...
},getChildren:function(X){return X.children
},getParentElement:function(X){return X.parentElement
},styleSheetRules:function(X){return X.rules
},getOuterHTML:function(X){return X.outerHTML
},setOuterHTML:function(X,Y){X.outerHTML=Y
},registerOverlayIframe:function(X){if(!V){V=new Object()
}V[X.id]=X
},deregisterOverlayIframe:function(X){if(V&&V[X.id]){V[X.id]=null
}},updateOverlayIframes:function(){if(this.IsIE6Only&&V){var a=V;
V=null;
for(var Y in a){var b=a[Y];
if(b){var X=true;
var Z=this.getElementById(b.divObjectId);
if(this.elementInDom(Z)&&this.effectiveStyle(Z,"display")!="none"&&Z.overlayIframe==b){X=false
}if(X){b.style.display="none"
}else{if(Z.awOnOverlayUpdate){Z.awOnOverlayUpdate(Z,b)
}}}}}},overlay:function(b,a){var Z;
var X;
var c;
if(this.IsIE6Only&&b){if(b.style.display=="none"||!b.currentStyle){return 
}if(b.currentStyle.zIndex==0){b.style.zIndex=100
}c=b.id+"IFrame";
Z=b.overlayIframe;
if(!Z){Z=this.getElementById(c)
}if(!Z){c=b.id+"IFrame";
X=document.createElement("span");
var Y=this.containerParent(b);
Y.appendChild(X);
X.innerHTML="<iframe src='"+this.AWEmptyDocScriptlet+"' id='"+c+"' style='position:absolute;top:0px;left:0px;display:none;filter:alpha(opacity=000);background-color:#FFFFFF'></iframe>";
Z=this.getElementById(c)
}b.overlayIframe=Z;
if(typeof (a)=="undefined"||a){Z.divObjectId=b.id;
this.registerOverlayIframe(Z)
}Z.style.width=b.offsetWidth;
Z.style.height=b.offsetHeight;
if(b.style.top){Z.style.top=b.style.top
}else{Z.style.top=this.absoluteTop(b)
}if(b.style.left){Z.style.left=b.style.left
}else{Z.style.left=this.absoluteLeft(b)
}Z.style.zIndex=b.currentStyle.zIndex-1;
Z.style.display="block";
if(!b.onresize){b.onresize=W
}}},unoverlay:function(Y){var X;
if(this.IsIE6Only&&Y){X=Y.overlayIframe;
if(Y.overlayIframe){X.style.display="none";
this.deregisterOverlayIframe(X);
Y.onresize=null
}}},EOF:0}
}())
}if(!A.IsIE){H.extend(A,function(){var V={IMG:true,BR:true,INPUT:true,META:true,LINK:true,PARAM:true,HR:true};
return{setPageScroll:function(X,W){window.scroll(X,W)
},setPageScrollTop:function(W){window.scroll(this.getPageScrollLeft(),W)
},getPageScrollTop:function(){return window.pageYOffset
},setPageScrollLeft:function(W){window.scroll(W,this.getPageScrollTop())
},getPageScrollLeft:function(){return window.pageXOffset
},documentElement:function(){return document.documentElement
},getChildren:function(W){return W.childNodes
},getParentElement:function(W){return W.parentNode
},styleSheetRules:function(W){return null
},getOuterHTML:function(Y){var W=Y.attributes;
var Z="<"+Y.tagName;
for(var X=0;
X<W.length;
X++){Z+=" "+W[X].name+'="'+W[X].value+'"'
}if(V[this.tagName]){return Z+">"
}return Z+">"+Y.innerHTML+"</"+Y.tagName+">"
},setOuterHTML:function(W,Y){var X=W.ownerDocument.createRange();
X.setStartBefore(W);
var Z=X.createContextualFragment(Y);
...
var W=null;
if(B.IsIE){V=document.createElement('<v:rect stroked="False">');
...
W.zIndex=Z;
return V
},coverDocument:function(Y,W){var V=this.createCoverDiv(Y,W);
...
U.cancelBubble(W)
}return{disableInput:function(W){if(B.isSafari&&!B.isIPad){return 
}var X=document.body;
...
}if(B.isSafari){setTimeout(function(){if(document.readyState!="loading"){window.location.href=c
}},500)
...
},displayErrorDiv:function(a){if(AWDebugEnabled){var b=document.createElement("div");
...
},hPassFocus:function(R,Q){H.removeClass(R.parentNode,"pfc");
J.focus(R)
